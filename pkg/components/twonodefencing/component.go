...
var TwoNodeFencingComponent = Component{
	Component: &config.Component{
		...
		TestRenames: map[string]string{
			...
			"[sig-etcd][apigroup:config.openshift.io][OCPFeatureGate:DualReplica][Suite:openshift/two-node][Serial][Disruptive] Two Node with Fencing etcd recovery should compute etcd revision bump and preserve backup container after kernel panic recovery": "[sig-etcd][apigroup:config.openshift.io][OCPFeatureGate:DualReplica][Suite:openshift/two-node][Serial][Disruptive] Two Node with Fencing etcd recovery should leave a backup container behind for debugging when etcd container crashes",
		},
	},
}
